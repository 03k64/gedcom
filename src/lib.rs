...
pub(crate) fn xref_id_to_numeric_id(xref_id: &str) -> Result<u32, &str> {
    let captures = XREF_ID_DIGITS
        .captures(xref_id)
        .ok_or("Could not find any digits in xref_id")?;

    let digits = captures
        .get(1)
        .ok_or("Could not find any digits in xref_id")?;

    let numeric_id = digits
        .as_str()
        .parse()
        .map_err(|_| "Could not parse digits in xref_id")?;
